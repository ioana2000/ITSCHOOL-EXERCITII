...
/*
Sa se scrie un program care verifica ca un numar este par sau impar.     
*/
let number = 13;
if (number % 2 === 0) {
  console.log('par');
...
function isPalindrome(word) {
    const length = word.length; // Inseamna lungimea cuvantului 
    
    for (let i = 0; i < length / 2; i++) {
         if (word[i] !== word[length - 1 - i]) {
            return false; //Daca gasim o diferenta nu este un palindrom 
         }
      }
      return true; //Daca toate caracterele sunt egale atunci este palindrom  
    }
    const word = "madam"; // Cuvantul 
    
    if (isPalindrome(word)) {
        console.log(`${word} este un palindrom.`);
    } else {
        console.log(`${word} nu este un palindrom.`);
    }


    function isPalindrome(word) {
      const length = word.length; // Determinăm lungimea cuvântului
      
      for (let i = 0; i < Math.floor(length / 2); i++) {
          if (word[i].toLowerCase() !== word[length - 1 - i].toLowerCase()) {
              return false; // Dacă găsim o diferență, nu este un palindrom
          }
      }
      return true; // Dacă toate caracterele sunt egale, atunci este palindrom
  }
  ...
  function checkPalindrome() {
      ...
      if (word) {
          if (isPalindrome(word)) {
              result.textContent = `${word} este un palindrom.`;
          } else {
              result.textContent = `${word} nu este un palindrom.`;
          }
      } else {
          result.textContent = "Vă rugăm să introduceți un cuvânt.";
      }
  }
  
  document.write(`
      <input type="text" id="wordInput" placeholder="Introduceți un cuvânt" />
      <button onclick="checkPalindrome()">Verifică</button>
      <p id="result"></p>
  `);